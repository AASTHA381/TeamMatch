...
- **Email sign-in required** — you sign in with a one-time link sent to any email you own (personal Gmail, Outlook, etc.), so only you can claim/edit your profile and no one else can overwrite your data
...
- The Firestore database is read-open (so matching/browsing works for everyone) but **writes require signing in via a one-time email link**, and each profile can only be edited by the account that first claimed it — enforced server-side by `firestore.rules`, not just client-side
...
- Firebase Authentication (passwordless email-link sign-in) + Firebase Firestore for the shared, real-time database
